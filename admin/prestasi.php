...
if ($action === 'view' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $res = mysqli_query($conn, "SELECT * FROM prestasi WHERE id = {$id} LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_prestasi'])) {
    ...
    header("Location: prestasi.php?msg=Data prestasi berhasil disimpan");
    exit;
}

/* ======================================================
   4. AMBIL DATA LIST PRESTASI (untuk mode list)
   ====================================================== */
$prestasiList = [];
if ($action === 'list') {
    ...
    if ($search !== '') {
        ...
        $sql = "SELECT * FROM prestasi ORDER BY tanggal DESC, id DESC";
    }
            ...
            <div class="content-card">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <th style="width:22%">Nama Prestasi</th>
                            ...
                            <tr>
                                <td>
                                    <?php if (!empty($row['foto'])): ?>
                                        <img src="<?= $row['foto_type'] ?>;base64,<?= base64_encode($row['foto']) ?>"
                                ...
                                <td>
                                    ...
                                    echo nl2br(htmlspecialchars(substr($desc, 0, 120) . (strlen($desc) > 120 ? '...' : '')));
                                    ?>
                                </td>
                                <td class="aksi-col">
        ...
        <?php elseif ($action === 'view' && $detail): ?>
            <div class="detail-container">
                <div class="detail-inner">
                    <div class="detail-back" onclick="window.location.href='prestasi.php'">⟵</div>

                    <?php if (!empty($detail['foto'])): ?>
                        <img src="<?= $detail['foto_type'] ?>;base64,<?= base64_encode($detail['foto']) ?>"
                             alt="Foto Prestasi" class="detail-image">
                    <?php else: ?>
                    ...
                    <div class="detail-meta">
                        <span><strong>Bidang:</strong> <?= htmlspecialchars($detail['bidang']) ?></span>
                        <span><strong>Penyelenggara:</strong> <?= htmlspecialchars($detail['penyelenggara'] ?? '-') ?></span>
                    </div>
        ...
        <?php elseif ($action === 'tambah' || $action === 'edit'): ?>
            <div class="content-card">
                <div class="header-row">
                    <div>
                        ...
                        <div class="breadcrumb">
                            Dashboard / Prestasi / 
                            <?= $action === 'tambah' ? 'Tambah' : 'Edit' ?> Prestasi
                        </div>
                    </div>
                </div>
                ...
                <form method="POST" enctype="multipart/form-data">
                            ...
                            <div class="upload-box" id="uploadBox">
                                <img id="previewImg" class="preview-img"
                                     src="<?= ($action === 'edit' && !empty($edit['foto']))
                                         ? '' . $edit['foto_type'] . ';base64,' . base64_encode($edit['foto'])
                                         : '' ?>"
                    ...
                    <button type="submit" name="save_prestasi" class="btn-simpan">
                        <i class="fas fa-save"></i>
                        <?= $action === 'edit' ? 'Update Data' : 'Simpan Data' ?>
                    </button>
                    <a href="prestasi.php" class="back-btn">&larr; Kembali ke Daftar</a>
                </form>
            </div>
        <?php endif; ?>
